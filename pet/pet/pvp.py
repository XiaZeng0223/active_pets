...
#             return SCIFACTPVP.VERBALIZER_A[label]
#         return SCIFACTPVP.VERBALIZER_B[label]
...
class CFEVERPVP(PVP):
    ...
    VERBALIZER_A = {
        "REFUTES": ["No"],
        "SUPPORTS": ["Yes"],
        "NOT_ENOUGH_INFO": ["Maybe"]
    }
    VERBALIZER_B = {
        "REFUTES": ["No"],
        "SUPPORTS": ["Yes"],
        "NOT_ENOUGH_INFO": ["Unsure"]
    }
    VERBALIZER_C = {
        "REFUTES": ["False"],
        "SUPPORTS": ["True"],
    ...
    def get_parts(self, example: InputExample) -> FilledPattern:
        ...
        if self.pattern_id in [0, 1, 2]:
            text_a = self.shortenable(self.remove_final_punc(example.text_a))
            text_b = self.shortenable(example.text_b)
            return ['"', text_a, '" ?'], [self.mask, ', "', text_b, '"']
        elif self.pattern_id in [3, 4, 5]:
            #swap claim and evidence
            text_a = self.shortenable(self.remove_final_punc(example.text_b))
            text_b = self.shortenable(example.text_a)
            return ['"', text_a, '" ?'], [self.mask, ', "', text_b, '"']

    def verbalize(self, label) -> List[str]:
        if self.pattern_id in [0, 3]:
            return CFEVERPVP.VERBALIZER_A[label]
        elif self.pattern_id in [1, 4]:
            return CFEVERPVP.VERBALIZER_B[label]
        elif self.pattern_id in [2, 5]:
            return CFEVERPVP.VERBALIZER_C[label]
...
class PubmedPVP(PVP):
    ...
    def verbalize(self, label) -> List[str]:
        return PubmedPVP.VERBALIZER[label]
