...
class MnliProcessor(DataProcessor):
    ...
    def _create_examples(lines: List[List[str]], set_type: str) -> List[InputExample]:
        ...
        for (i, line) in enumerate(lines):
            ...
            text_a = line[8]    #hypothesis
            text_b = line[9]    #premise
            label = line[-1]
...
class CFEVERProcessor(DataProcessor):
    """Processor for the CFEVER data set."""
    ...
    def _create_examples(self, path: str, set_type: str, hypothesis_name: str = "claim",
                         premise_name: str = "evidence") -> List[InputExample]:
        examples = []
        ...
        with open(path, encoding='utf8') as f:
            for line_idx, line in enumerate(f):
                ...
                text_a = example_json[hypothesis_name]
                guid = "%s-%s" % (set_type, idx)
                text_b = example_json[premise_name]
...
class PUBMEDProcessor(DataProcessor):
    ...
    def get_labels(self):
        return ["BACKGROUND", "OBJECTIVE", "METHODS", "RESULTS", "CONCLUSIONS"]
...
PROCESSORS = {
    "mnli": MnliProcessor,
    "scifact_oracle": CFEVERProcessor,
    "scifact": CFEVERProcessor,
    "cfever":CFEVERProcessor,
...
METRICS = {
    "cb": ["acc", "f1-macro"],
    "multirc": ["acc", "f1", "em"],
    "scifact":["acc", "f1-macro", "f1"],
    "scifact_oracle": ["acc", "f1-macro", "f1"],
    "cfever":["acc", "f1-macro", "f1"],
    "pubmed":['acc', 'f1-macro', 'f1']
}
