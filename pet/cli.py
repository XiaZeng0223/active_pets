...
from pet.tasks import PROCESSORS, load_examples, TRAIN_SET, DEV_SET, TEST_SET, METRICS, DEFAULT_METRICS, UNLABELED_SET
...
def main():
    ...
    parser.add_argument("--sampled_dir", default=None, type=str,
                        help="The input data dir. Should contain the sampled train data file for the task.")
    parser.add_argument("--oversampling", action='store_true',
                        help="Do oversampling: duplicate data from less sampled class to fake the balance.")
    ...
    parser.add_argument('--sampling_seed', type=int, default=42,
                        help="random seed for training sample selection")
    ...
    parser.add_argument("--eval_set", choices=['train', 'dev', 'test'], default='dev',
    ...
    if args.sampled_dir:
        train_data = load_examples(
            args.task_name, args.data_dir, TRAIN_SET, num_examples=train_ex, num_examples_per_label=train_ex_per_label, seed=args.sampling_seed)
        # Train only on sampled examples
        sampled_file = os.path.join(args.output_dir, 'sampled.pt')
        os.makedirs(args.output_dir, exist_ok=True)
        sampled = torch.load(sampled_file).numpy()

        # oversampling
        if args.oversampling:
            sampled_label = [train_data[s].label for s in sampled]
            label_distribution = Counter(sampled_label)
            print('sampled', sampled)
            logger.info('original label distribution: %s', label_distribution)
            s= [j for i, j in enumerate(sampled) if sampled_label[i] == 'SUPPORTS']
            n= [j for i, j in enumerate(sampled) if sampled_label[i] == 'NOT_ENOUGH_INFO']
            c= [j for i, j in enumerate(sampled) if sampled_label[i] == 'REFUTES']
            k = max(len(s), len(n), len(c))

            oversampled = []
            for cls in [s, n, c]:
                if cls !=[]:
                    oversampled.extend(np.resize(cls, k).tolist())

            np.random.shuffle(oversampled)
            print('oversampled', oversampled)
            sampled_data = Subset(train_data, oversampled)
            print('sampled_data', sampled_data)

            logger.info("Sampled %s train data", len(sampled_data))
            ...
            logger.info('original label distribution: %s', label_distribution)
            sampled_data = Subset(train_data, sampled)
            logger.info("Sampled %s train data", len(sampled_data))
            train_data = sampled_data
    else:
        train_data = load_examples(
    ...
    # df_train= pd.DataFrame([InputExample.to_dict(x) for x in train_data])
    # outdir = os.path.join(os.getcwd(), args.output_dir)
    # os.makedirs(outdir, exist_ok=True)
    # df_train.to_csv(os.path.join(outdir, 'train_data_{}.csv'.format(args.sampling_seed))
    ...
    if args.method == 'pet':
        pet.train_pet(pet_model_cfg, pet_train_cfg, pet_eval_cfg, sc_model_cfg, sc_train_cfg, sc_eval_cfg,
                      ...
                      no_distillation=args.no_distillation, seed=args.seed, save_model=args.save_model)
