...
def main():
    ...
    parser.add_argument("--eval_set", choices=['train', 'dev', 'test'], default='train',
    ...
    mapping = {'train': TRAIN_SET, 'dev':DEV_SET, 'test':TEST_SET}
    eval_set = mapping[args.eval_set]
    ...
    #     train_data = Subset(train_data, sampled)
    #     logger.info("Using %s sampled train data", len(train_data))
    # else:
    #     train_data = load_examples(
    #         args.task_name, args.data_dir, TRAIN_SET, num_examples=train_ex, num_examples_per_label=train_ex_per_label, seed=args.sampling_seed)
